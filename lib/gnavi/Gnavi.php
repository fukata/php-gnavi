...
class Gnavi {
    ...
    public function restSearchAPI($query=array()) {
        $qs = http_build_query($query);
        $content = file_get_contents(self::API_BASE . "/RestSearchAPI/?$qs");
        $xml = $this->_parseRestSearchAPI($content);
        return $xml;
    }
    ...
    public function _parseRestSearchAPI($content) {
        ...
        $data = (object) array(
            'total_hit_count' => $xml->total_hit_count,
            'hit_per_page' => $xml->hit_per_page,
            'rest' => array(),
        );
        if ( ! isset( $xml->rest[0] ) ) {
